module "eks" {
  ...
  access_entries = var.endpoint_access == "private" ? {
    ...
    bastion_admin = {
      principal_arn = local.bastion_role_arn  #one(data.aws_iam_role.bastion_role[*].arn) #try(aws_iam_role.bastion_role.arn, null) #aws_iam_role.bastion_role.arn  # Your existing bastion role
      policy_associations = {
        admin = {
  ...
  eks_managed_node_groups = {
    primary_ng = {
      ...
      iam_role_name            = "${local.cluster_name}-ng-primary"
